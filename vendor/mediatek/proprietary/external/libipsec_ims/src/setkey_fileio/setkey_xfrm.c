...
void set_property_volte()
{
	...
	if(strstr(getprogname(),"volte")!= NULL)
		if(volte_pid ==-1)
		{
			if (sprintf(pid,"%d",getpid()) < 0)
			{
				ALOGE("sprintf failed,errno:%d\n",errno);
				return;
			}
                 ...
                 char * encrp_algo_src,char * encrp_key_src,char * intergrity_algo_src,char * intergrity_key_src,int u_id)
{
	...
	if(port_head == NULL)
	{
		strncpy(src_arr,ip_src,strlen(ip_src));
		src_arr[strlen(ip_src)] = '\0';
	...
	if(port_head==NULL)
	{
		strncpy(dst_arr,ip_dst,strlen(ip_dst));
		dst_arr[strlen(ip_dst)] = '\0';
...
int setkey_SP_xfrm(int cmd,char * src_range,char * dst_range,enum PROTOCOL_TYPE protocol,char * port_src,char * port_dst,char * src_tunnel,char * dst_tunnel,char * ipsec_type,char * mode, char * direction,int u_id)
{
	...
	if(src_range) {
		strncpy(src_arr,src_range,strlen(src_range));
		src_arr[strlen(src_range)] = '\0';
	}
	if(dst_range) {
		strncpy(dst_arr,dst_range,strlen(dst_range));
		dst_arr[strlen(dst_range)] = '\0';
	}

	if(src_tunnel) {
		strncpy(src_tunnel_arr,src_tunnel,strlen(src_tunnel));
		src_tunnel_arr[strlen(src_tunnel)] = '\0';
	}
	if(dst_tunnel) {
		strncpy(dst_tunnel_arr,dst_tunnel,strlen(dst_tunnel));
		dst_tunnel_arr[strlen(dst_tunnel)] = '\0';
	}
