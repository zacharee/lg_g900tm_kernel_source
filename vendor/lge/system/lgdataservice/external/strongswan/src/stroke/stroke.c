...
static int trigger_dpd_now()
{
	...
	msg = create_stroke_msg(STR_DPD_NOW);
	return send_stroke_msg(msg);
}
...
static int trigger_dpd_now_per_conn(char *conn_name, int retransmit_tries, int retransmit_timeout)
{
	...
	msg = create_stroke_msg(STR_DPD_NOW_PER_CONN);
	...
	switch (token->kw)
	{
		...
		case STROKE_DPD:
			...
			if (argc == 1)
				...
				res = trigger_dpd_now_per_conn(argv[1], 0, 0);
			else if (argc == 4)
				res = trigger_dpd_now_per_conn(argv[1], atoi(argv[2]), atoi(argv[3]));
			else
			/* 2016-07-02 protocol-iwlan@lge.com LGP_DATA_IWLAN_DPD_NOW [END] */
			res = trigger_dpd();
			break;
