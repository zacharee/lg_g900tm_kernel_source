...
static int get_psi(char* name) {
    ...
    DBG1(DBG_IKE, "get_psi psi name : %s", name);
...
/* LGP_DATA_IWLAN Support DEVICE_IDENTITY [START] */
static uint8_t get_imei_check_digit(const char *imei) {
	int i = 0;
	unsigned int sum = 0;
	uint8_t digit = 0;
	int last_digit_idx = 14; /* Check the first 14 IMEI digits (idx: 0 to 13) and */
	                         /* write the check digit at the 15th position (idx: 14) */

	while (i < last_digit_idx) {
		if (i % 2) {
			digit = (imei[i] - 48) *  2;
			if (digit >= 10) {
				digit -= 9; /* the sum of the digits between 10 and 18 is the same as subtraction by 9 */
			}
			sum += digit;
		}
		else {
	...
	/* sum % 10 is between 0 and 9 */
	/* 10 minus the above number is to get the value to satisfy n mod 10 == 0 */
	/* final result is between 1 and 10 so mod 10 is done again */
	return (uint8_t) (10 - (sum % 10)) % 10;
}

static chunk_t build_device_identity(const char *imei, const char *svn, uint8_t device_identity_type)
{
	chunk_t chunk = chunk_empty;
	size_t i;

	int last_idx = 8; /* IMEI LENGTH: ceil(15/2) == ceil(16/2) == 8 */
	u_char *bin = malloc(last_idx + 1); /* +1 space for Device Identity Type - the last_idx is at bin[8] */

	int last_digit_idx = 14; /* Position the index after the 14th digit of the IMEI (idx: 0 to 13), which is 14 */
	...
	if (device_identity_type == 1) {
		if (imei_len == 15) {
			bin[0] = 0x1;
			bin[last_idx] = (imei[last_digit_idx] - 48) + (0xF << 4);
		}
		else if (imei_len == 16) {
			bin[0] = 0x1;
			bin[last_idx] = get_imei_check_digit(imei) + (0xF << 4);
		}
		else {
			bin[0] = 0x0;
		}
	}
	else if (device_identity_type == 2) {
		if (imei_len == 15) {
			if (svn) {
				svn_len = strlen(svn);
			}
			...
			if (svn_len == 2) {
				bin[0] = 0x2;
				bin[last_idx] = (svn[0] - 48) + ((svn[1] - 48) << 4);
			}
			else {
				bin[0] = 0x0;
			}
			...
			bin[0] = 0x2;
			bin[last_idx] = (imei[last_digit_idx] - 48) + (imei[last_digit_idx + 1] << 4);
		}
		else {
			bin[0] = 0x0;
		}
	...
	if (bin[0] != 0x0) {
		/* Copy the middle of the IMEI */
		for (i = 0; i < last_digit_idx; i += 2) {
...
static void add_device_identiy_notify(private_ike_auth_t *this, message_t *message)
{
	uint8_t device_identity_type;
	peer_cfg_t *config;
	int slotid = get_slotid(this->ike_sa->get_name(this->ike_sa));

	device_identity_type = this->ike_sa->get_device_identity_type(this->ike_sa);
	if (device_identity_type != 0)
	{
		config = this->ike_sa->get_peer_cfg(this->ike_sa);
		if (config->get_imei(config))
		{
			notify_payload_t *notify;
			chunk_t notification_data, notification_length, imei;
			u_char *bin = malloc(2);

			bin[0] = 0x0;
			bin[1] = 0x9;
			notification_length = chunk_create(bin, 2);

			imei = build_device_identity(config->get_imei(config), config->get_svn(config), device_identity_type);

			if(imei.len != 0)
			{
				notification_data = chunk_cat("mm", notification_length, imei);


				notify = notify_payload_create(PLV2_NOTIFY);
				notify->set_notify_type(notify, ME_DEVICE_IDENTITY);
				notify->set_protocol_id(notify, PROTO_IKE);
				notify->set_notification_data(notify, notification_data);
				message->add_payload(message, (payload_t*)notify);
			}
			else
			{
				chunk_free(&notification_length); /* bin is freed as well */
			}
		}
		this->ike_sa->set_device_identity_type(this->ike_sa, chunk_empty); /* clean the value */
	}
}
/* LGP_DATA_IWLAN Support DEVICE_IDENTITY [END] */
	...
	private_ike_auth_t *this, message_t *message)
{
	...
	{	/* in the first IKE_AUTH ... */
		...
		if (get_cust_setting_int_by_slotid(slotid, EPDG_SUPPORT_5GS) == 1)
		{
            ...
            DBG2(DBG_CFG, "[5GS] EPDG_SUPPORT_5GS , psi ==> %d", psi);
...
/* LGP_DATA_IWLAN Support DEVICE_IDENTITY [START] */
	else if (message->get_message_id(message) == 2)
	{
		add_device_identiy_notify(this, message);
	}
/* LGP_DATA_IWLAN Support DEVICE_IDENTITY [END] */
	...
	while (enumerator->enumerate(enumerator, &payload))
	{
		if (payload->get_type(payload) == PLV2_NOTIFY)
		{
			...
			switch (type)
			{
				...
				//LGP_DATA_IWLAN support_5gs [END]
				//LGP_DATA_IWLAN Support DEVICE_IDENTITY [START]
				case ME_DEVICE_IDENTITY:
					this->ike_sa->set_device_identity_type(this->ike_sa, notify->get_notification_data(notify));
					break;
				//LGP_DATA_IWLAN Support DEVICE_IDENTITY [END]
				default:
				{
