...
/* 2021-01-20 lgsi-epdg-data@lge.com LGP_DATA_IWLAN [START] */
#include <utils/cust_settings.h>
#ifdef __ANDROID__
#include <cutils/properties.h>
#include "wod_channel.h"
#endif
/* 2021-01-20 lgsi-epdg-data@lge.com LGP_DATA_IWLAN [END] */
		...
		if (payload->get_type(payload) == PLV2_NOTIFY)
		{
			...
			switch (type)
			{
				...
				case USE_BEET_MODE:
				{
					...
					snprintf(value, PROP_VALUE_MAX, "%d", type);
					...
					if ((strcmp(status, "0") == 0) && (property_set("net.wo.statuscode", value) != 0))
					{
						DBG1(DBG_IKE, "property_set fail with ('net.wo.statuscode', value)");
					}
#endif
					break;
				}
