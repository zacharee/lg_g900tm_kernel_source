					   ...
					   int family, uint16_t *port)
{
	...
	if (skt < 0)
	{
       ...
       // In some devices, the ePDG gateway address may be: (::ffff:192.168.1.XXX). To solve such an issue, add the following socket option.
       // Reference : RFC3493 Realated TC : AT&T LTE-BTR-5-7168
       if (family == AF_INET6)
       {
              if (setsockopt(skt, IPPROTO_IPV6, IPV6_V6ONLY, (void *)&on, sizeof(on)) < 0)
              {
