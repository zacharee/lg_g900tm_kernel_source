	...
	private_ike_config_t *this, message_t *message)
{
	...
	{	/* in first IKE_AUTH only */
		...
		if (config->get_imei(config)) {
			...
			int slotid = get_slotid(this->ike_sa->get_name(this->ike_sa));
			int cust_imei_cp_value = get_cust_setting_int_by_slotid(slotid, CUST_IMEI_CP);
			DBG1(DBG_IKE, "CUST_IMEI_CP: [%d]", cust_imei_cp_value);
			//to comply with private use values as defined by IANA
			if ((0 < cust_imei_cp_value) && (cust_imei_cp_value <= 0x7FFF)) {
				if(!cp) {
					cp = cp_payload_create_type(PLV2_CONFIGURATION, CFG_REQUEST);
				}
			...
			{
				DBG1(DBG_IKE, "Attribute type value doesn't comply with private use values defined by IANA");
			}
