...
static void pop_end(stroke_msg_t *msg, const char* label, stroke_end_t *end)
{
	...
	/* LGP_DATA_IWLAN Support DEVICE_IDENTITY [START] */
	DBG1(DBG_CFG, "  %ssvn=%s", label, end->svn);
	/* LGP_DATA_IWLAN Support DEVICE_IDENTITY [END] */
...
static void stroke_add_conn(private_stroke_socket_t *this, stroke_msg_t *msg)
{
	...
	DBG_WITH_ANDROID_PROPERTY( "persist.product.lge.data.service.privacy.enable", DBG_NET, "MY ID '%s'", msg->add_conn.me.id);
