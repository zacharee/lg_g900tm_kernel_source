...
METHOD(child_sa_manager_t, get_sa, ike_sa_t*,
	private_child_sa_manager_t *this, protocol_id_t protocol, uint32_t spi, host_t *dst)
{
	...
	if (entry)
	{
		return charon->ike_sa_manager->get_sa_by_ikeid_rekey(charon->ike_sa_manager,id);
	}
	return NULL;
}
