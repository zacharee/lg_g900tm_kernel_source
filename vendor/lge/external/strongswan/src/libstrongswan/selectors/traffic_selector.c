	...
	private_traffic_selector_t *this, host_t *host)
{
	...
	else
	{
		...
		memcpy(this->to, from.ptr, from.len);
                //LGP_DATA_IWLAN set_address reflects host's netbits to this traffic_selector.
                //this->netbits = from.len * 8;
                patch_code_id("LPCP-1776@n@c@libstrongswan@traffic_selector.c@2");
		...
		private_traffic_selector_t *this)
{
	...
	if (this != NULL && this->type == TS_IPV4_ADDR_RANGE) {
		memset(this->from, 0x00, sizeof(uint32_t));
		memset(this->to, 0xFF, sizeof(uint32_t));
		this->netbits = 0;
	} else if (this != NULL && this->type == TS_IPV6_ADDR_RANGE) {
		memset(this->from, 0x00, sizeof(uint32_t)*4);
		memset(this->to, 0xFF, sizeof(uint32_t)*4);
		this->netbits = 0;
	}
}
	...
	private_traffic_selector_t *this, host_t *host)
{
	...
	if (host->is_anyaddr(host))
	{
		memset(this->from, 0x00, IPV6_LEN);
		memset(this->to, 0xFF, IPV6_LEN);
		this->netbits = 0;
	}
	else
	{
