...
int main(int argc, char **argv) {
  ...
  unsigned len;
  /* 2020-05-07 minkeun.kwon@lge.com LGP_DATA_PERFORMANCE_CLAT [START] */
  patch_code_id("LPCP-1812@n@c@clatd@main.c@1");
  char propVal [PROPERTY_VALUE_MAX];
  memset(propVal, 0x0, PROPERTY_VALUE_MAX);
  int ret = property_get("persist.product.lge.data.lgclat.enabled", propVal, "false");
  if(ret > 0 && (strncasecmp(propVal, "true", 4) == 0 || strncasecmp(propVal, "1", 1) == 0)) {
    LG_CLAT_ENABLED = 1;
  } else {
    LG_CLAT_ENABLED = 0;
  }
  ...
  /* 2020-05-07 minkeun.kwon@lge.com LGP_DATA_PERFORMANCE_CLAT [START] */
  patch_code_id("LPCP-1812@n@c@clatd@main.c@2");
  logmsg(ANDROID_LOG_INFO, "Starting clat version %s on %s netid=%s mark=%s plat=%s v4=%s v6=%s, %s",
         CLATD_VERSION, uplink_interface, net_id_str ? net_id_str : "(none)",
         mark_str ? mark_str : "(none)", plat_prefix ? plat_prefix : "(none)",
         v4_addr ? v4_addr : "(none)", v6_addr ? v6_addr : "(none)", LG_CLAT_ENABLED ? "[LG]" : "");
  /* 2020-05-07 minkeun.kwon@lge.com LGP_DATA_PERFORMANCE_CLAT [END] */
