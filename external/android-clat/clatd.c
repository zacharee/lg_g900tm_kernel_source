...
void configure_tun_ip(const struct tun_data *tunnel, const char *v4_addr) {
  ...
  patch_code_id("LPCP-1812@n@c@clatd@clatd.c@1");
  if(LG_CLAT_ENABLED) {
...
void open_sockets(struct tun_data *tunnel, uint32_t mark) {
  ...
  /* 2020-05-07 minkeun.kwon@lge.com LGP_DATA_PERFORMANCE_CLAT [START] */
  patch_code_id("LPCP-1812@n@c@clatd@clatd.c@2");
  if (LG_CLAT_ENABLED) {
    int packetsock = socket(AF_PACKET, SOCK_DGRAM | SOCK_NONBLOCK | SOCK_CLOEXEC, htons(ETH_P_IPV6));
    if (packetsock < 0) {
...
void configure_interface(const char *uplink_interface, const char *plat_prefix, const char *v4_addr,
  ...
  /* 2020-05-07 minkeun.kwon@lge.com LGP_DATA_PERFORMANCE_CLAT [START] */
  patch_code_id("LPCP-1812@n@c@clatd@clatd.c@3");
  if(LG_CLAT_ENABLED) {
 ...
 * tunnel    - tun device data
 */
void read_packet_lgclat(int active_fd, const struct tun_data *tunnel) {
  ssize_t readlen;
  uint8_t buf[PACKETLEN], *packet;
  int fd, numread = 0;
...
void event_loop(struct tun_data *tunnel) {
  ...
  while (running) {
    if (poll(wait_fd, ARRAY_SIZE(wait_fd), NO_TRAFFIC_INTERFACE_POLL_FREQUENCY * 1000) == -1) {
      ...
      /* 2020-05-07 minkeun.kwon@lge.com LGP_DATA_PERFORMANCE_CLAT [START] */
      patch_code_id("LPCP-1812@n@c@clatd@clatd.c@4");
      if(LG_CLAT_ENABLED) {
        size_t i;
        for(i = 0; i < ARRAY_SIZE(wait_fd); i++) {
          ...
          if(wait_fd[i].revents != 0) {
            read_packet_lgclat(wait_fd[i].fd,tunnel);
          }
        }
      } else {
