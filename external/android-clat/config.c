...
/* 2020-05-07 minkeun.kwon@lge.com LGP_DATA_PERFORMANCE_CLAT [START] */
/* function: config_item_int32_t
 ...
 * ret_val_ptr - pointer for return value storage
 */
int32_t *config_item_int32_t(cnode *root, const char *item_name, const char *defaultvar, int32_t *ret_val_ptr) {
  const char *tmp;
  char *endptr;
  long int conf_int;
...
/* 2020-05-07 minkeun.kwon@lge.com LGP_DATA_PERFORMANCE_CLAT [END] */
...
int read_config(const char *file, const char *uplink_interface, const char *plat_prefix,
  ...
  /* 2020-05-07 minkeun.kwon@lge.com LGP_DATA_PERFORMANCE_CLAT [START] */
  patch_code_id("LPCP-1812@n@c@clatd@config.c@1");
  if(LG_CLAT_ENABLED) {
    if(!config_item_int32_t(root, "send_buffer", "0", &Global_Clatd_Config.send_buffer))
      goto failed;

    if(!config_item_int32_t(root, "receive_buffer", "0", &Global_Clatd_Config.receive_buffer))
      goto failed;

    if(!config_item_int32_t(root, "packet_burst", DEFAULT_PACKET_BURST, &Global_Clatd_Config.packet_burst))
      goto failed;

    if (Global_Clatd_Config.packet_burst == 0) {
      logmsg(ANDROID_LOG_FATAL, "invalid packet_burst %d", Global_Clatd_Config.packet_burst);
      goto failed;
    }
  }
  /* 2020-05-07 minkeun.kwon@lge.com LGP_DATA_PERFORMANCE_CLAT [END] */
