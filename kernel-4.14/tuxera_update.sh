...
gen_header_checksums() {
    ...
    command -v ${KERNEL_COMPILER} > /dev/null 2>&1
    ...
    command -v ${KERNEL_LINKER} > /dev/null 2>&1
...
lookup_cache() {
    ...
    for pkg in ${cachefiles} ; do
        ...
        for driver_obj in "${cache_dir}/pkgtmp/"*/objects ; do
            ...
            SILENT_BUILD="1>/dev/null 2>/dev/null"
            if [ "${verbose}" == "yes" ] ; then
            ...
            make -C "$kernel" ${CACHE_MAKE_FLAGS} M="${driver}/objects" modules ${SILENT_BUILD}
