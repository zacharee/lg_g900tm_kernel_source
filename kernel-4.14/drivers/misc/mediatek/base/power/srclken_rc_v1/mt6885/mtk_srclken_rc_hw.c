...
/* Dynamic XO_NFC Clock control */
int sls_nfc_clk_on(void)
{
	int ret = 0;
	...
	if (rc_stage_init_done) {
		...
		u32 id = SYS_NFC;
		u32 mode = SW_MODE;
		u32 req = FPM_REQ;		/*req as parameter input*/

		if (req == NO_REQ)
			req = (srclken_read(RC_M00_SRCLKEN_CFG + 4 * id) & (FPM_REQ | BBLPM_REQ));
		srclken_write(RC_M00_SRCLKEN_CFG + 4 * id,
			(srclken_read(RC_M00_SRCLKEN_CFG + 4 * id)
			& ~(bit_mask)) | req | mode);
		msleep(2);

		/* pr_info("read back value.(0x%x)", srclken_read(RC_M00_SRCLKEN_CFG + 4 * id)); */
		pr_info("%s: turn on xo_nfc by SW mode\n", __func__);
	} else {
		ret = -1;
		pr_info("%s: scrlken not init yet\n", __func__);
	}
