				...
				unsigned long event, void *data)
{
	...
	case TCP_NOTIFY_SINK_VBUS:
		...
		if ((rpmd->sink_mv_new != rpmd->sink_mv_old) ||
			...
			if (rpmd->sink_mv_new && rpmd->sink_ma_new) {
				 ...
				 */
				charger_dev_enable(rpmd->wless_charger, false);
#else /* MediaTek */
				 ...
				 * to mt_charger_set_property() in mtk_chg_type_det.c
				 */
				charger_dev_enable(rpmd->wless_charger, true);
#else /* MediaTek */
				charger_manager_enable_power_path(
	...
	case TCP_NOTIFY_TYPEC_STATE:
		...
		dev_info(rpmd->dev,
			 "%s cc1: %d cc2: %d\n", __func__,
			 rpmd->tcpc->typec_remote_cc[0], rpmd->tcpc->typec_remote_cc[1]);
		typec_set_partner_cc_status(rpmd->typec_port,
			rpmd->tcpc->typec_remote_cc[0], rpmd->tcpc->typec_remote_cc[1]);
	#endif
		if (old_state == TYPEC_UNATTACHED &&
			...
			#ifdef CONFIG_LGE_USB_TYPE_C
			typec_set_pd_rev(rpmd->typec_port, TYPEC_PD_REV_NOT_SUPPORTED);
			#endif
		...
		if (new_state == TYPEC_UNATTACHED) {
			...
			if (rpmd->typec_caps.prefer_role == TYPEC_SOURCE) {
				...
				typec_set_pwr_opmode(rpmd->typec_port,
						     TYPEC_PWR_MODE_USB);
				#ifdef CONFIG_LGE_USB_TYPE_C
				typec_set_pd_rev(rpmd->typec_port,
							TYPEC_PD_REV_NOT_SUPPORTED);
				#endif
	...
	case TCP_NOTIFY_PD_STATE:
		...
		case PD_CONNECT_PE_READY_SRC_PD30:
			...
			#ifdef CONFIG_LGE_USB_TYPE_C
			typec_set_pd_rev(rpmd->typec_port,
				(rpmd->typec_caps.pd_revision>>8)&0xFF);
			#endif
			if (!rpmd->partner)
				break;
	...
	case TCP_NOTIFY_CABLE_TYPE:
		#ifdef CONFIG_LGE_USB_TYPE_C
		dev_info(rpmd->dev, "%s cable type = [%s]\n",
				    __func__, pd_cable_string[noti->cable_type.type]);
		#else
	...
	case TCP_NOTIFY_LGE_MOSITURE_STATE:
		dev_info(rpmd->dev, "%s moisture status %d\n", __func__,
			 rpmd->tcpc->is_mositure_detected);;
		noti->m_status.status = rpmd->tcpc->is_mositure_detected;
		typec_set_moisture(rpmd->typec_port, noti->m_status.status);
		break;
	#endif
...
#ifdef CONFIG_LGE_PM_WIRELESS_CHARGER
	rpmd->wless_charger = get_charger_by_name("wless_chg");
	if (!rpmd->wless_charger)
