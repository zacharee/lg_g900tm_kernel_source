...
//#define DS3_DEBUG		// DUAL_SCREEN DEBUG
...
#ifdef DS3_DEBUG
void print_svid_value(struct svdm_svid_data *svid_data)
{
	int i;
	pr_info("START DS3_DEBUG: %s -----\n", __func__);
	pr_info("svid exit: %d svid: %x active_mode: %x\n",
		svid_data->exist, svid_data->svid, svid_data->active_mode);

	pr_info("svid->local.vdo_cnt: %d\n", svid_data->local_mode.mode_cnt);
	pr_info("svid->remote.vdo_cnt: %d\n", svid_data->remote_mode.mode_cnt);

	if (svid_data->local_mode.mode_cnt) {
		for (i=0; i< svid_data->local_mode.mode_cnt; i++) {
				pr_info("svid->local.vdo[%d]: %x\n", i, svid_data->local_mode.mode_vdo[i]);
			}
	}

	if (svid_data->remote_mode.mode_cnt) {
		for (i=0; i< svid_data->local_mode.mode_cnt; i++) {
				pr_info("svid->remote.vdo[%d]: %x\n", i, svid_data->local_mode.mode_vdo[i]);
			}
	}
	pr_info("END DS3_DEBUG: %s -----\n", __func__);
}

void print_pd_port_value(struct pd_port *pd_port)
{
	int i;
	pr_info("START DS3_DEBUG: %s -----\n", __func__);
	...
	pr_info("svid_data_cnt: %d\n", pd_port->svid_data_cnt);
	//pr_info("payload: 0x%x\n", pd_port->pd_msg_data_payload);

	pr_info("dpm_caps: 0x%x\n", pd_port->dpm_caps);

	pr_info("ufp_u_state:%d dfp_u_state:%d\n",
		pd_port->pe_data.dp_data.dfp_u_state, pd_port->pe_data.dp_data.ufp_u_state);

	pr_info("local_status:0x%x remote_status:0x%x\n",
		pd_port->pe_data.dp_data.local_status, pd_port->pe_data.dp_data.remote_status);

	pr_info("local_config:0x%x remote_config:0x%x\n",
		pd_port->pe_data.dp_data.local_config, pd_port->pe_data.dp_data.remote_config);
		...
		struct pd_port *pd_port, struct svdm_svid_data *svid_data)
{
	...
	#ifdef DS3_DEBUG
	pr_info("%s DS3_DEBUG\n",__func__);
	print_svid_value(svid_data);
	print_pd_port_value(pd_port);
	#endif
