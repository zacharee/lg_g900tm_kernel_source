...
#include <soc/mediatek/lge/board_lge.h> /* LGE_CHANGE, 2020-04-29, revision check for reva, ssora.lee@lge.com */
...
/* LGE_CHANGE_S, 2020-04-29, revision check for reva, ssora.lee@lge.com */
	if(lge_get_board_revno() == HW_REV_A) {
		pr_info("###imgsensor_hw camera custom i2c REV_A");
		pcust_pwr_cfg = imgsensor_custom_config_reva;
	} else {
		pr_debug("###imgsensor_hw camera custom i2c not REV_A");
