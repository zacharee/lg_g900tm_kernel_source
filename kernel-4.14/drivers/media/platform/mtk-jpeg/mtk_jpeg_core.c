		 ...
		 struct mtk_jpeg_enc_param *config)
{
	...
	//unsigned int core_id = ctx->coreid;//error: unused variable 'core_id' [-Werror,-Wunused-variable]
	struct mtk_jpeg_dev *jpeg = ctx->jpeg;
...
{
	struct mtk_jpeg_dev *jpeg = ctx->jpeg;
	pr_info("%s  +", __func__);//move down error: ISO C90 forbids mixing declarations and code [-Werror,-Wdeclaration-after-statement]
