...
int battery_aging_set_factor_level(unsigned int condition)
{
	if (condition)
		set_factor_level = condition;
	else
		set_factor_level = 0;

	pr_info("[AGING] battery_aging_set_factor_level: %d condition: %d\n",
			set_factor_level, condition);
...
int battery_aging_get_factor_level(void)
{
	...
	unsigned int condition_age = 100;

	if (set_factor_level)
		return set_factor_level;

	if (!battery_aging_factor)
		return condition_age;

	condition_age = battery_aging_factor / 1000;
	condition_age += 1;
	condition_age *= 10;

	if (condition_age >= 100)
		condition_age = 100;

	if (condition_age >= 90)
		value = 1;
	else if (condition_age >= 60)
		value = 2;
	else
		value = 3;

	return condition_age + value;
}
