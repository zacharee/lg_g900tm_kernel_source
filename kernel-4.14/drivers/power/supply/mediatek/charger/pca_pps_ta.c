...
static struct prop_chgalgo_desc pca_desc = {
	.name = "pca_ta_pps",
	.type = PCA_DEVTYPE_TA,
};
...
static int pca_pps_probe(struct platform_device *pdev)
{
	...
	info->pca = prop_chgalgo_device_register(info->dev, &pca_desc,
						 &pca_pps_ops, NULL, NULL,
						 info);
	if (!info->pca) {
		dev_err(info->dev, "%s register pps fail\n", __func__);
		return -EINVAL;
	}
...
static int __init pca_pps_notifier_call_init(void)
{
	int ret = 0;
	struct prop_chgalgo_device *pca =
		prop_chgalgo_dev_get_by_name("pca_ta_pps");
	struct pca_pps_info *info = NULL;

	if (!pca) {
		pr_err("%s get pca fail\n", __func__);
		return -EINVAL;
	}

	info = prop_chgalgo_get_drvdata(pca);
	...
	ret = register_tcp_dev_notifier(info->tcpc, &info->tcp_nb,
					TCP_NOTIFY_TYPE_USB);
	if (ret < 0)
		dev_err(info->dev, "%s register tcpc notifier fail(%d)\n",
				   __func__, ret);
	else
		dev_info(info->dev, "%s successfully\n", __func__);
...
late_initcall(pca_pps_notifier_call_init);
