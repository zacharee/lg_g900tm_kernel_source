...
extern int store_aod_area(struct mtk_dsi *dsi, struct lge_rect *rect);
...
static void lge_set_custom_rgb_sw43103(struct mtk_dsi *dsi, bool send_cmd, unsigned int need_lock)
{
	int i = 0;
	...
	*/
	...
	if (send_cmd) {
		lge_ddic_dsi_panel_tx_cmd_set(dsi, LGE_DDIC_DSI_DISP_DG_COMMAND_DUMMY, need_lock);
	}
...
static void lge_display_control_store_sw43103(struct mtk_dsi *dsi, bool send_cmd, unsigned int need_lock)
{
	...
	if (send_cmd) {
		lge_ddic_dsi_panel_tx_cmd_set(dsi, LGE_DDIC_DSI_DISP_CTRL_COMMAND_1, need_lock);
	}
...
static void lge_set_screen_tune_sw43103(struct mtk_dsi *dsi, unsigned int need_lock)
{
	int i;
	int sat_index = 0;
	int hue_index = 0;
	...
	lge_ddic_dsi_panel_tx_cmd_set(dsi, LGE_DDIC_DSI_SET_SATURATION, need_lock);
	lge_ddic_dsi_panel_tx_cmd_set(dsi, LGE_DDIC_DSI_SET_HUE, need_lock);
	lge_ddic_dsi_panel_tx_cmd_set(dsi, LGE_DDIC_DSI_SET_SHARPNESS, need_lock);
...
static void lge_set_screen_mode_sw43103(struct mtk_dsi *dsi, bool send_cmd, unsigned int need_lock)
{
	struct drm_crtc *crtc = NULL;
	struct mtk_drm_private *private = NULL;
	struct mtk_panel_ext *panel = NULL;
	...
	case screen_mode_oled_natural:
		...
		lge_ddic_dsi_panel_tx_cmd_set(dsi, LGE_DDIC_DSI_CM_NATURAL, need_lock);

		mutex_unlock(&panel->panel_lock);
		COMMIT_UNLOCK(need_lock, &private->commit.lock);
		lge_set_custom_rgb_sw43103(dsi, send_cmd, need_lock);
		COMMIT_LOCK(need_lock, &private->commit.lock);
		mutex_lock(&panel->panel_lock);
		break;
	case screen_mode_oled_vivid:
		...
		panel->lge.sharpness_status = 0x00;
		pr_info("vivid mode\n");
		lge_ddic_dsi_panel_tx_cmd_set(dsi, LGE_DDIC_DSI_CM_VIVID, need_lock);
		break;
	case screen_mode_oled_cinema:
		panel->lge.dgc_status = 0x01;
		panel->lge.sharpness_status = 0x00;
		pr_info("cinema mode\n");
		lge_ddic_dsi_panel_tx_cmd_set(dsi, LGE_DDIC_DSI_CM_CINEMA, need_lock);
		break;
	case screen_mode_oled_custom:
		...
		lge_ddic_dsi_panel_tx_cmd_set(dsi, LGE_DDIC_DSI_SET_SATURATION_DEFAULT, need_lock);
		lge_ddic_dsi_panel_tx_cmd_set(dsi, LGE_DDIC_DSI_SET_HUE_DEFAULT, need_lock);
		lge_ddic_dsi_panel_tx_cmd_set(dsi, LGE_DDIC_DSI_SET_WB_DEFAULT, need_lock);

		mutex_unlock(&panel->panel_lock);
		COMMIT_UNLOCK(need_lock, &private->commit.lock);
		lge_set_screen_tune_sw43103(dsi, need_lock);
		lge_set_custom_rgb_sw43103(dsi, send_cmd, need_lock);
		COMMIT_LOCK(need_lock, &private->commit.lock);
		mutex_lock(&panel->panel_lock);
		break;
	...
	lge_display_control_store_sw43103(dsi, send_cmd, need_lock);
	return;
}

static void lge_set_fp_lhbm_sw43103(struct mtk_dsi *dsi, int input, unsigned int need_lock)
{
	struct mtk_panel_ext *panel = NULL;
	struct drm_crtc *crtc = NULL;
	struct mtk_drm_private *private = NULL;
	...
	COMMIT_LOCK(need_lock, &private->commit.lock);
	mutex_lock(&panel->panel_lock);
	fp_aod_ctrl = lge_dsi_panel_is_power_on_lp(dsi);
	...
	case LGE_FP_LHBM_READY:
		...
		lge_ddic_dsi_panel_tx_cmd_set(dsi, LGE_DDIC_DSI_FP_LHBM_READY, need_lock);	//Enable dimming
	...
	case LGE_FP_LHBM_FORCED_ON:
		if(fp_aod_ctrl) {
			lge_ddic_dsi_panel_tx_cmd_set(dsi, LGE_DDIC_DSI_FP_LHBM_AOD_TO_FPS, need_lock);
			pr_info("AOD to Normal mode\n");
		}
		pr_info("[LHBM ON] set max brightness 0x6A9\n");
		lge_ddic_dsi_panel_tx_cmd_set(dsi, LGE_DDIC_DSI_FP_LHBM_ON, need_lock);
	...
	case LGE_FP_LHBM_FORCED_OFF:
		lge_ddic_dsi_panel_tx_cmd_set(dsi, LGE_DDIC_DSI_FP_LHBM_OFF, need_lock);
		if (fp_aod_ctrl) {
			lge_ddic_dsi_panel_tx_cmd_set(dsi, LGE_DDIC_DSI_FP_LHBM_FPS_TO_AOD, need_lock);
			pr_info("Normal to AOD mode\n");
		}
		if (!panel->lge.lhbm_ready_enable) {
			pr_info("[LHBM OFF] current brightness = %d\n", panel->lge.bl_level);
#ifdef CONFIG_MTK_AAL_SUPPORT
			printk("[LHBM OFF] set brightness directly\n");
			...
			bl_payload[1] = (panel->lge.bl_level >> 8) & 0x07;
			bl_payload[2] = panel->lge.bl_level & 0xFF;

			lge_ddic_dsi_panel_tx_cmd_set(dsi, LGE_DDIC_DSI_BL_SET, need_lock);
#else
			mt_leds_brightness_set("lcd-backlight", panel->lge.bl_level);
#endif
		}
		panel->lge.forced_lhbm = false;
	break;
	...
	case LGE_FP_LHBM_FORCED_EXIT:
		...
		panel->lge.forced_lhbm = false;
#ifdef CONFIG_MTK_AAL_SUPPORT
		printk("[LHBM EXIT] set brightness directly\n");
		bl_payload = get_payload_addr(panel, LGE_DDIC_DSI_BL_SET, 0);
		if (!bl_payload) {
			pr_err("LGE_DDIC_DSI_BL_SET is NULL\n");
			goto error;
		...
		lge_ddic_dsi_panel_tx_cmd_set(dsi, LGE_DDIC_DSI_FP_LHBM_OFF, need_lock);		//force off to avoid abnormal case
		lge_ddic_dsi_panel_tx_cmd_set(dsi, LGE_DDIC_DSI_FP_LHBM_EXIT, need_lock);	//Disable dimming

		if (lge_dsi_panel_is_power_on_interactive(dsi)) { // TO do
			mutex_unlock(&panel->panel_lock);
			COMMIT_UNLOCK(need_lock, &private->commit.lock);
			lge_set_screen_mode_sw43103(dsi,true, need_lock);
			COMMIT_LOCK(need_lock, &private->commit.lock);
			mutex_lock(&panel->panel_lock);
		}
	break;
...
static void lge_update_ddic_hdr_status(struct mtk_panel_ext *panel)
{
	panel->lge.ddic_hdr = !!(panel->lge.ve_hdr | panel->lge.ace_hdr);
	pr_info("hdr %d ve_hdr %d ace_hdr %d \n", panel->lge.ddic_hdr, panel->lge.ve_hdr, panel->lge.ace_hdr);
}

static void lge_set_video_enhancement_sw43103(struct mtk_dsi *dsi, int input, unsigned int need_lock)
{
	int rc = 0;
	struct drm_crtc *crtc = NULL;
	struct mtk_drm_private *private = NULL;
	...
	if (enable) {
		rc = lge_ddic_dsi_panel_tx_cmd_set(dsi, LGE_DDIC_DSI_VIDEO_ENHANCEMENT_ON, need_lock);
		if (rc)
			pr_err("failed to send VIDEO_ENHANCEMENT_ON cmd, rc=%d\n", rc);
	}
	else {
		rc = lge_ddic_dsi_panel_tx_cmd_set(dsi, LGE_DDIC_DSI_VIDEO_ENHANCEMENT_OFF, need_lock);
		if (rc)
...
static void lge_set_ecc_status_sw43103(struct mtk_dsi *dsi, int input, unsigned int need_lock)
{
	int rc = 0;
	struct drm_crtc *crtc = NULL;
	struct mtk_drm_private *private = NULL;
	...
	mutex_lock(&panel->panel_lock);
	if (input) {
		rc = lge_ddic_dsi_panel_tx_cmd_set(dsi, LGE_DDIC_DSI_ECC_STATUS_ON, need_lock);
		if (rc)
			pr_err("failed to send ECC_STATUS_ON cmd, rc=%d\n", rc);
	} else {
		rc = lge_ddic_dsi_panel_tx_cmd_set(dsi, LGE_DDIC_DSI_ECC_STATUS_OFF, need_lock);
		if (rc)
...
static void lge_update_irc_state(struct mtk_panel_ext *panel, int value)
{
...
int lge_set_irc_state_sw43103(struct mtk_dsi *dsi, enum lge_irc_mode mode, enum lge_irc_ctrl enable,unsigned int need_lock)
{
	char *payload = NULL;
	struct drm_crtc *crtc = NULL;
	struct mtk_drm_private *private = NULL;
	...
	lge_ddic_dsi_panel_tx_cmd_set(dsi, LGE_DDIC_DSI_IRC_CTRL, need_lock);
	...
	if ((mode == LGE_GLOBAL_IRC_HBM) && panel->lge.use_ace_ctrl) {
		if (enable == LGE_IRC_OFF) {
			panel->lge.ace_hdr = 1;
			lge_update_ddic_hdr_status(panel);
			lge_ddic_dsi_panel_tx_cmd_set(dsi, LGE_DDIC_DSI_ACE_TUNE, need_lock);
			mutex_unlock(&panel->panel_lock);
		} else {
			panel->lge.ace_hdr = 0;
			lge_update_ddic_hdr_status(panel);
			lge_ddic_dsi_panel_tx_cmd_set(dsi, LGE_DDIC_DSI_ACE_RESTORE, need_lock);
			mutex_unlock(&panel->panel_lock);
			lge_set_screen_mode_sw43103(dsi, true, 0);
		}
		COMMIT_UNLOCK(need_lock, &private->commit.lock);
		return 0;
	}
...
int lge_get_irc_state_sw43103(struct mtk_dsi *dsi)
{
	struct mtk_panel_ext *panel = NULL;
	int ret;
...
int lge_set_irc_default_state_sw43103(struct mtk_dsi *dsi)
{
	struct mtk_panel_ext *panel = NULL;
...
static int lge_daylight_mode_set_sw43103(struct mtk_dsi *dsi, int input, unsigned int need_lock)
{
	struct drm_crtc *crtc = NULL;
	struct mtk_drm_private *private = NULL;
	struct mtk_panel_ext *panel = NULL;
	...
	if(panel->lge.daylight_mode)
		lge_ddic_dsi_panel_tx_cmd_set(dsi, LGE_DDIC_DSI_DAYLIGHT_ON, need_lock);
	else
		lge_ddic_dsi_panel_tx_cmd_set(dsi, LGE_DDIC_DSI_DAYLIGHT_OFF, need_lock);
	mutex_unlock(&panel->panel_lock);
	COMMIT_UNLOCK(need_lock, &private->commit.lock);
	pr_info("daylight_mode = %d\n", panel->lge.daylight_mode);

	if (panel->lge.hdr_mode) {
		lge_set_irc_state_sw43103(dsi, LGE_GLOBAL_IRC_HDR, false, need_lock);
		lge_display_control_store_sw43103(dsi, true, need_lock);
	} else {
		lge_set_irc_state_sw43103(dsi, LGE_GLOBAL_IRC_HDR, true, need_lock);
		lge_set_screen_mode_sw43103(dsi, true, need_lock);
	}
...
static int lge_hdr_mode_set_sw43103(struct mtk_dsi *dsi, int input, unsigned int need_lock)
{
	...
	if (panel->lge.use_irc_ctrl) {
		if (hdr_mode)
			lge_set_irc_state_sw43103(dsi, LGE_GLOBAL_IRC_HDR, LGE_IRC_OFF,need_lock);
		else
			lge_set_irc_state_sw43103(dsi, LGE_GLOBAL_IRC_HDR, LGE_IRC_ON,need_lock);
	}
...
static void lge_vr_lp_mode_set_sw43103(struct mtk_dsi *dsi, int input, unsigned int need_lock)
{
	int rc = 0;
	bool enable = false;
	struct drm_crtc *crtc = NULL;
	...
	if (enable) {
		rc = lge_ddic_dsi_panel_tx_cmd_set(dsi, LGE_DDIC_DSI_SET_VR_MODE_ON, need_lock);
	} else {
		rc = lge_ddic_dsi_panel_tx_cmd_set(dsi, LGE_DDIC_DSI_SET_VR_MODE_OFF, need_lock);
	}
	mutex_unlock(&panel->panel_lock);
	COMMIT_UNLOCK(need_lock, &private->commit.lock);
...
static void lge_set_tc_perf_sw43103(struct mtk_dsi *dsi, int input, unsigned int need_lock)
{
	...
	mutex_lock(&panel->panel_lock);
	if (tc_perf_mode)
		lge_ddic_dsi_panel_tx_cmd_set(dsi, LGE_DDIC_DSI_TC_PERF_ON_COMMAND, need_lock);
	else
		lge_ddic_dsi_panel_tx_cmd_set(dsi, LGE_DDIC_DSI_TC_PERF_OFF_COMMAND, need_lock);
	mutex_unlock(&panel->panel_lock);
	COMMIT_UNLOCK(need_lock, &private->commit.lock);
