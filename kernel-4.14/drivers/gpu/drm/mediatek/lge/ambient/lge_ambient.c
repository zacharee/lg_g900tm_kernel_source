...
static void set_interface_data(struct mtk_panel_ext *panel, int *data)
{
...
int lge_ambient_set_interface_data(struct mtk_panel_ext *panel)
{
	int i;
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	rect = kzalloc(sizeof(struct lge_rect), GFP_KERNEL);
	if(!rect) {
		pr_err("[%s] failed to alloc memory for struct lge_rect\n", __func__);
		return ret;
	}
	...
	if (panel->lge.ddic_ops && panel->lge.ddic_ops->store_aod_area) {
		...
		panel->lge.ddic_ops->store_aod_area(dsi, rect);
...
void lge_ambient_create_sysfs(struct mtk_dsi *dsi, struct class *class_panel)
{
	static struct device *aod_sysfs_dev = NULL;

	if(!aod_sysfs_dev){
		aod_sysfs_dev = device_create(class_panel, NULL, 0, dsi, "aod");
