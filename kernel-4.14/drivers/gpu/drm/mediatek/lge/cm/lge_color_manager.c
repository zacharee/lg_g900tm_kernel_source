		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	pr_info("daylight mode : %d \n", panel->lge.daylight_mode);

	if (panel->lge.ddic_ops && panel->lge.ddic_ops->daylight_mode_set)
		panel->lge.ddic_ops->daylight_mode_set(dsi, panel->lge.daylight_mode, 1);
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (panel->lge.ddic_ops && panel->lge.ddic_ops->hdr_mode_set)
		panel->lge.ddic_ops->hdr_mode_set(dsi, panel->lge.hdr_mode, 1);
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (panel->lge.ddic_ops->sharpness_set)
		panel->lge.ddic_ops->sharpness_set(dsi, input);
	else
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (panel->lge.ddic_ops && panel->lge.ddic_ops->lge_set_screen_mode)
		panel->lge.ddic_ops->lge_set_screen_mode(dsi, true, 1);
	else
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (panel->lge.ddic_ops && panel->lge.ddic_ops->lge_set_screen_tune)
		panel->lge.ddic_ops->lge_set_screen_tune(dsi, 1);

	panel->lge.sharpness_status = 0x01;

	if (panel->lge.ddic_ops && panel->lge.ddic_ops->lge_display_control_store)
		panel->lge.ddic_ops->lge_display_control_store(dsi, true, 1);
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (panel->lge.color_manager_default_status) {
		...
		if (panel->lge.ddic_ops && panel->lge.ddic_ops->lge_set_custom_rgb)
			panel->lge.ddic_ops->lge_set_custom_rgb(dsi, true, 1);
	} else {
		if (panel->lge.ddic_ops && panel->lge.ddic_ops->lge_set_rgb_tune)
			panel->lge.ddic_ops->lge_set_rgb_tune(dsi, true);
	}
						...
						struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (panel->lge.color_manager_status && panel->lge.ddic_ops && panel->lge.ddic_ops->lge_send_screen_mode_cmd)
		panel->lge.ddic_ops->lge_send_screen_mode_cmd(dsi, panel->lge.color_manager_mode);
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (panel->lge.ddic_ops && panel->lge.ddic_ops->lge_set_hdr_hbm_lut)
		panel->lge.ddic_ops->lge_set_hdr_hbm_lut(dsi, panel->lge.hdr_hbm_lut);
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (panel->lge.ddic_ops && panel->lge.ddic_ops->lge_set_acl_mode)
		panel->lge.ddic_ops->lge_set_acl_mode(dsi, panel->lge.acl_mode);
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (panel->lge.ddic_ops && panel->lge.ddic_ops->lge_set_video_enhancement)
		panel->lge.ddic_ops->lge_set_video_enhancement(dsi, panel->lge.video_enhancement, 1);
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (panel->lge.ddic_ops && panel->lge.ddic_ops->lge_set_ecc_status)
		panel->lge.ddic_ops->lge_set_ecc_status(dsi, panel->lge.ecc_status,1);
...
void lge_mdss_dsi_bc_dim_work(struct work_struct *work)
{
	...
	panel = container_of(lge_panel, struct mtk_panel_ext, lge);
	if (!panel) {
...
void lge_bc_dim_work_init(struct mtk_panel_ext *panel)
{
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (panel->lge.ddic_ops && panel->lge.ddic_ops->lge_set_therm_dim)
		panel->lge.ddic_ops->lge_set_therm_dim(dsi, input);
	else
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (panel->lge.ddic_ops && panel->lge.ddic_ops->lge_get_brightness_dim)
		bc_dim_f_cnt = panel->lge.ddic_ops->lge_get_brightness_dim(dsi);
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (panel->lge.ddic_ops && panel->lge.ddic_ops->lge_set_brightness_dim)
		panel->lge.ddic_ops->lge_set_brightness_dim(dsi, input);
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (panel->lge.ddic_ops && panel->lge.ddic_ops->lge_vr_lp_mode_set)
		panel->lge.ddic_ops->lge_vr_lp_mode_set(dsi, enable, 1);
	else
		pr_info("Not support\n");
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (panel->lge.ddic_ops && panel->lge.ddic_ops->lge_set_true_view_mode)
		panel->lge.ddic_ops->lge_set_true_view_mode(dsi, true);
	else
...
int lge_color_manager_create_sysfs(struct mtk_panel_ext *panel, struct device *panel_sysfs_dev)
{
	int rc = 0;
...
int lge_dsi_parse_color_manager_modes(struct device_node *np,
