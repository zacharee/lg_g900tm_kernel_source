...
extern int lge_dsi_panel_tx_cmd_set(struct mtk_dsi *dsi,
...
extern int lge_dsi_panel_cmd_read(struct mtk_dsi *dsi, u8 cmd, int cnt, char* ret_buf, unsigned int need_lock);
						...
						const char *buf, size_t len)
{
	...
	if (!rc && count) {
		...
		if (!lge_dsi_panel_parse_cmd_sets_sub(&cmd, data, count)) {
			...
			if (lge_dsi_panel_tx_cmd_set(dsi, &cmd, 1))
						...
						const char *buf, size_t len)
{
	...
	lge_dsi_panel_cmd_read(dsi, (u8)cmd, cnt, &reg[0], 1);
	atomic_set(&idlemgr->idlemgr_task_active, 1);

	if (panel->lge.use_ddic_reg_lock)
		lge_ddic_dsi_panel_tx_cmd_set(dsi, LGE_DDIC_DSI_REGISTER_LOCK, 1);
...
void lge_panel_reg_create_sysfs(struct mtk_dsi *dsi, struct class *class_panel)
{
	static struct device *panel_reg_dev = NULL;

	if (!panel_reg_dev) {
		panel_reg_dev = device_create(class_panel, NULL, 0, dsi, "reg");
