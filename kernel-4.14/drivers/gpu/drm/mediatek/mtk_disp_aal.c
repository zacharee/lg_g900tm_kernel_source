...
static atomic_t g_aal_backlight_notified = ATOMIC_INIT(2047);
...
static atomic_t g_aal_backlight_stored = ATOMIC_INIT(0);
...
void disp_aal_notify_backlight_changed(int bl_1024)
{
	...
	if (bl_1024 == 0) {
		...
		atomic_set(&g_aal_backlight_stored, bl_1024);
	...
	struct drm_file *file_priv)
{
	...
	if (atomic_read(&g_aal_backlight_stored) != backlight_value)
		mt_leds_brightness_set("lcd-backlight", backlight_value);

	atomic_set(&g_aal_backlight_stored, backlight_value);
