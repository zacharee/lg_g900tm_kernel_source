...
#endif
		drm_dp_dpcd_read(&mtk_dp->aux, DPCD_00200, ubTempValue, 0x1);
...
int mdrv_DPTx_SetTrainingStart(struct mtk_dp *mtk_dp)
{
	...
	//mhal_DPTx_SetTxLaneToLane(mtk_dp, 0,1);
	//mhal_DPTx_SetTxLaneToLane(mtk_dp, 1,0);
#endif
		...
		struct drm_display_mode *mode)
{
	...
	if (fakecablein || (mode->hdisplay == 3840 && mode->vdisplay == 2160 &&
