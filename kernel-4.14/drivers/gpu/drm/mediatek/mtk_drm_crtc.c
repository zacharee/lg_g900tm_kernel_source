	...
	for (i = 0; i < CRTC_PROP_MAX; i++) {
		if (private->crtc_property[index][i] == property) {
			...
			if (i == CRTC_PROP_PRES_FENCE_IDX)
				DDPFENCE("S+/%s%d/PF/idx%d\n",
		 			index ? "E" : "P", index, (unsigned int)val);
	...
	struct cmdq_pkt *cmdq_handle)
{
	...
	{
		int old_idx = old_mtk_state->prop_val[CRTC_PROP_DISP_MODE_IDX];
		int new_idx = mtk_state->prop_val[CRTC_PROP_DISP_MODE_IDX];

		if (mtk_crtc->panel_ext && mtk_crtc->panel_ext->funcs && mtk_crtc->panel_ext->funcs->ext_param_get) {
			struct mtk_panel_params param = { 0, };
			if (!mtk_crtc->panel_ext->funcs->ext_param_get(&param, old_idx))
				fps_src = param.dyn_fps.vact_timing_fps;
			if (!mtk_crtc->panel_ext->funcs->ext_param_get(&param, new_idx))
				fps_dst = param.dyn_fps.vact_timing_fps;
		}
		if (mode->vrefresh < fps_dst) {
			mtk_crtc->panel_ext->lge.refresh_rate_div = fps_dst / mode->vrefresh;
			pr_info("%s: fps: %d(%d) -> %d(%d) refresh_rate_div=%d\n", __func__, crtc->state->mode.vrefresh, fps_src, mode->vrefresh, fps_dst, mtk_crtc->panel_ext->lge.refresh_rate_div);
		} else {
			mtk_crtc->panel_ext->lge.refresh_rate_div = 0;
		}
	}
#endif
...
static int mtk_drm_pf_release_thread(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		if (MTK_SESSION_TYPE(private->session_id[crtc_idx])&0x3) {
			mtk_release_present_fence(private->session_id[crtc_idx],
						  fence_idx);
		} else {
			pr_err_ratelimited("invalid session id:0x%08x\n", private->session_id[crtc_idx]);
		}
