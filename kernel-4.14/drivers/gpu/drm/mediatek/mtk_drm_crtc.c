...
int mtk_drm_setbacklight(struct drm_crtc *crtc, unsigned int level, unsigned int need_lock)
{
	struct mtk_drm_crtc *mtk_crtc = to_mtk_crtc(crtc);
	...
	if (!(mtk_crtc->enabled)) {
		DDPMSG("Sleep State set backlight stop --crtc not ebable\n");
		if (need_lock)
	...
	struct drm_crtc *crtc, int session_id)
{
	...
	for (i = 0; i < mtk_crtc->layer_nr; i++) {
		...
		subtractor &= 0xFFFF;
#if 0
		if (drm_crtc_index(crtc) == 1 && subtractor == 0
		    && fence_idx > 0 && mtk_crtc->enabled) {
			DDPINFO("crtc1, layer%d,fence_idx:%d,subtractor:%d\n",
