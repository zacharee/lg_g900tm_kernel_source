...
{
	if (!merge_config->height || !merge_config->width_left || !merge_config->width_right) {
		DDPPR_ERR("%s:merge input width l(%u) w(%u) h(%u)\n",
			  __func__, merge_config->width_left, merge_config->width_right, merge_config->height);
		return -EINVAL;
	}
		       ...
		       struct cmdq_pkt *handle)
{
	...
	cmdq_pkt_write(handle, comp->cmdq_base,
		       ...
		       merge_config.width_left | (merge_config.width_right << 16), ~0);
	cmdq_pkt_write(handle, comp->cmdq_base,
