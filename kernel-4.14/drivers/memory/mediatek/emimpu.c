...
static void (*md_handling_cb)(
	unsigned int emi_id, struct reg_info_t *dump, unsigned int leng);
static irqreturn_t (*tmem_handling_cb)(
	unsigned int emi_id, struct reg_info_t *dump, unsigned int leng);
...
/*
 * mtk_emimpu_tmem_handling_register - register callback for pre-fetch handling
 * @tmem_handling_func:   function point for pre-fetch handling
 *
 * Return 0 for success, -EINVAL for fail
 */
int mtk_emimpu_tmem_handling_register(
	irqreturn_t (*tmem_handling_func)
	(unsigned int emi_id, struct reg_info_t *dump, unsigned int leng))
{
	if (!tmem_handling_func) {
		pr_info("%s: tmem_handling_func is NULL\n", __func__);
		return -EINVAL;
	}

	tmem_handling_cb = tmem_handling_func;
	return 0;
}
EXPORT_SYMBOL(mtk_emimpu_tmem_handling_register);
