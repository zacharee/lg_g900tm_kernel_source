...
static int ufstw_auto_ee(struct ufsf_feature *ufsf)
{
	...
	if (hba->ee_ctrl_mask & mask){
		err = EINVAL;//error: variable 'err' is used uninitialized whenever 'if' condition is true [-Werror,-Wsometimes-uninitialized]
		goto out;
	}
...
void ufstw_suspend(struct ufsf_feature *ufsf)
{
	struct ufstw_lu *tw;
	int lun;
	//int ret; //error: unused variable 'ret' [-Werror,-Wunused-variable]
