...
{
	return single_open(file, rg_ssusb_idrvsel_show, PDE_DATA(inode));
}

static int rg_ssusb_idemsel_open(struct inode *inode, struct file *file)
{
	return single_open(file, rg_ssusb_idemsel_show, PDE_DATA(inode));
}
...
int mtu3_phy_init_debugfs(struct phy *phy)
{
	...
	file = proc_create_data(FILE_RG_SSUSB_IDRVSEL, 0644,
				   root, &rg_ssusb_idrvsel_fops, phy);
	if (!file) {
		ret = -ENOMEM;
		goto err1;
	}

	file = proc_create_data(FILE_RG_SSUSB_IDEMSEL, 0644,
				   root, &rg_ssusb_idemsel_fops, phy);
	if (!file) {
		ret = -ENOMEM;
		goto err1;
	}
