		...
		size_t count, loff_t *ppos)
{
	...
	rc = anx7418_read_reg(i2c, DEVICE_ID_H);
	rc <<= 8;
	rc |= anx7418_read_reg(i2c, DEVICE_ID_L);
	rc <<= 8;
	rc |= anx7418_read_reg(i2c, DEVICE_VERSION);
	temp += scnprintf(buf + temp, buf_size - temp, "DEVICE: %X\n", rc);

	rc = anx7418_read_reg(i2c, ANALOG_CTRL_3);
	...
	rc = anx7418_read_reg(i2c, POWER_DOWN_CTRL);
	temp += scnprintf(buf + temp, buf_size - temp,
			"POWER_DOWN_CTRL(%02x)\n", rc);
	...
	#if 0 /* Need not type-c status */
	rc = anx7418_read_reg(i2c, ANALOG_STATUS);
	temp += scnprintf(buf + temp, buf_size - temp,
			"ANALOG_STATUS(%02x)\n", rc);
	temp += scnprintf(buf + temp, buf_size - temp,
			"\tDFP_OR_UFP: %s\n",
	...
	rc = anx7418_read_reg(i2c, ANALOG_CTRL_0);
	temp += scnprintf(buf + temp, buf_size - temp,
			"ANALOG_CTRL_0(%02x)\n", rc);
	temp += scnprintf(buf + temp, buf_size - temp,
			"\t%s\n",
	...
	rc = anx7418_read_reg(i2c, ANALOG_CTRL_6);
	temp += scnprintf(buf + temp, buf_size - temp,
			"ANALOG_CTRL_6(%02x)\n", rc);
	switch (rc & R_RP) {
	case 0x00: // 36k
	...
	rc = anx7418_read_reg(i2c, ANALOG_CTRL_7);
	temp += scnprintf(buf + temp, buf_size - temp,
			"ANALOG_CTRL_7(%02x)\n", rc);
	if (rc & CC2_5P1K)
		temp += scnprintf(buf + temp, buf_size - temp,
	...
	rc = anx7418_read_reg(i2c, ANALOG_CTRL_1);
	if (rc & BIT(0))
		temp += scnprintf(buf + temp, buf_size - temp,
				"\tml0p --a11, ml0n --a10\n");
	if (rc & BIT(1))
	...
	if (rc & BIT(7))
		temp += scnprintf(buf + temp, buf_size - temp,
				"\tml1p --a2,  ml1n --a3\n");
	rc = anx7418_read_reg(i2c, ANALOG_CTRL_5);
	if (rc & BIT(4))
		temp += scnprintf(buf + temp, buf_size - temp,
				"\tWarning - Reserved ANX7418\n");
	if (rc & BIT(5))
	...
	rc = anx7418_read_reg(i2c, ANALOG_CTRL_2);
	if (rc & BIT(0))
		temp += scnprintf(buf + temp, buf_size - temp,
				"\tSafe Mode\n");
	else
		...
		size_t count, loff_t *ppos)
{
	...
	for (i = 0; i < 256; i += 16) {
		temp += scnprintf(buf + temp, buf_size - temp,
				"%.8x: ", i);
		for (j = 0; j < 16; j++) {
			rc = anx7418_read_reg(anx->i2c, i + j);
#if 0
			if (rc < 0) {
		...
		size_t count, loff_t *ppos)
{
	...
	val = anx7418_read_reg(anx->i2c, dbgfs_reg);
	if (val < 0)
		rc = scnprintf(buf, buf_size, "%d\n", val);
	else
		rc = scnprintf(buf, buf_size, "%02x\n", val);
		...
		size_t count, loff_t *ppos)
{
	...
	rc = anx7418_write_reg(anx->i2c, dbgfs_reg, val);
	if (rc < 0)
		goto err;
 ...
 * anx7418_dbgfs_mode
 */
static ssize_t anx7418_dbgfs_mode_read(struct file *file, char __user *ubuf,
		size_t count, loff_t *ppos)
{
	struct anx7418_dbgfs *dbgfs = file->private_data;
	struct anx7418 *anx = dbgfs->anx;
...
static ssize_t anx7418_dbgfs_mode_write(struct file *file, const char __user *ubuf,
		size_t count, loff_t *ppos)
{
	struct anx7418_dbgfs *dbgfs = file->private_data;
	struct anx7418 *anx = dbgfs->anx;
	...
	anx7418_update_cross_switch(val);
	if (rc < 0)
		goto err;
...
static const struct file_operations anx7418_dbgfs_mode_ops = {
	.open = anx7418_dbgfs_open,
	.read = anx7418_dbgfs_mode_read,
	.write = anx7418_dbgfs_mode_write,
	.release = anx7418_dbgfs_release,
};
...
int anx7418_debugfs_init(struct anx7418 *anx)
{
	...
	entry = debugfs_create_file("mode", 0600, anx7418_dbgfs_dent,
				anx, &anx7418_dbgfs_mode_ops);
	if (!entry)
		goto err;
