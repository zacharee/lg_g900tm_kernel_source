...
static s32 tusb546_i2c_xfer(struct i2c_adapter *adapter, u16 addr,
...
s32 __tusb546_read_reg(const struct i2c_client *client, u8 command)
{
	...
	status = tusb546_i2c_xfer(client->adapter, client->addr, client->flags,
		...
		u8 value)
{
	union i2c_smbus_data data;
	data.byte = value;
	return tusb546_i2c_xfer(client->adapter, client->addr, client->flags,
...
int tusb546_read_reg(struct i2c_client *client, u8 reg)
{
	...
	i2c_lock_adapter(client->adapter);
	rc = __tusb546_read_reg(client, reg);
	i2c_unlock_adapter(client->adapter);
...
int tusb546_write_reg(struct i2c_client *client, u8 reg, u8 val)
{
	...
	i2c_lock_adapter(client->adapter);
	rc = __tusb546_write_reg(client, reg, val);
	i2c_unlock_adapter(client->adapter);
		...
		u8 reg, u8 len, u8 *data)
{
	...
	i2c_lock_adapter(client->adapter);
	rc = __tusb546_read_block_reg(client, reg, len, data);
	i2c_unlock_adapter(client->adapter);
		...
		u8 reg, u8 len, const u8 *data)
{
	...
	i2c_lock_adapter(client->adapter);
	rc = __tusb546_write_block_reg(client, reg, len, data);
	i2c_unlock_adapter(client->adapter);
