...
static void fusb251_print_reg(int reg, u8 value)
{
	...
	case FUSB251_SWITCH_CONTROL:
		pr_info("FUSB251 SWITCH_CTRL - CC:%d, SBU:0x%x\n",
				(value & CC_SW_CTRL_BIT) ? 1 : 0,
				value & (u8)SBU_SW_CTRL_MASK);
		break;
...
void fusb251_set_factory_mode(bool fmode)
{
	int value = 0;
	...
	pr_info("%s: SWITCH: current [0x%x]\n",__func__, value);

	if (fmode)
		value = (value & 0xF9) | SBU1_FM_SBU2_OPEN;
	else
		value = (value & 0xF9) | CLOSE_SBU1_SBU2;
...
EXPORT_SYMBOL(fusb251_set_factory_mode);
...
void fusb251_enable_moisture(bool enable)
{
	...
	if (enable) {
		...
		#ifndef CONFIG_LGE_USB_TUSB546
		else
		#endif
#endif
#ifdef CONFIG_LGE_USB_TUSB546
		#ifdef CONFIG_LGE_DUAL_SCREEN
		else if (is_tusb546_dp())
			fusb251_set_state(fusb, FUSB251_STATE_DP_CONNECTED);
		else
		#else
		if (is_tusb546_dp())
			fusb251_set_state(fusb, FUSB251_STATE_DP_CONNECTED);
...
static int psy_changed(struct notifier_block *nb, unsigned long evt, void *ptr)
{
...
#ifdef CONFIG_TCPC_MT6360
	vbus_present = tcpm_inquire_vbus_level(fusb->tcpc, true) >= TCPC_VBUS_VALID ? true : false;
	typec_mode = tcpm_inquire_typec_attach_state(fusb->tcpc);
#else
	...
	if (fusb->vbus_present != vbus_present) {
		...
		if (fusb->vbus_present) {
			...
			if (!fusb->ds_connected && !fusb->dp_connected) {
				schedule_delayed_work(&fusb->enable_mos_work, 0);
				fusb251_set_state(fusb, FUSB251_STATE_DRY);
			} else {
				if (fusb->dp_connected)
					fusb251_set_state(fusb, FUSB251_STATE_DP_CONNECTED);
				else
					fusb251_set_state(fusb, FUSB251_STATE_DS_CONNECTED);
			}
...
static void fusb251_set_state(struct fusb251 *fusb, int state)
{
	...
	if (fusb->state == state) // return function if the state is not changed.
		return;
	...
	case FUSB251_STATE_DS_CONNECTED:
		...
		kick_sm(fusb, 0);
		break;
	case FUSB251_STATE_DP_CONNECTED:
		fusb->dp_connected = true;
		kick_sm(fusb, 0);
		break;
...
static void fusb251_sm(struct work_struct *w)
{
...
#ifdef CONFIG_TCPC_MT6360
		/* SOS Change */
		/* tcpm_notify_moisture_detection(fusb->tcpc, false); */
		tcpci_notify_moisture_detection(fusb->tcpc, false);
#else
		val.intval = 0;
...
#ifdef CONFIG_TCPC_MT6360
		ret = tcpm_typec_change_role(fusb->tcpc, TYPEC_ROLE_TRY_SNK);
#else
...
#ifdef CONFIG_TCPC_MT6360
			ret = tcpm_typec_change_role(fusb->tcpc, TYPEC_ROLE_SNK);
#else
...
#ifdef CONFIG_TCPC_MT6360
		/* SOS Change */
		/* tcpm_notify_moisture_detection(fusb->tcpc, true); */
		tcpci_notify_moisture_detection(fusb->tcpc, true);
		ret = tcpm_typec_change_role(fusb->tcpc, TYPEC_ROLE_SNK);
#else
		val.intval = 1;
		ret = power_supply_set_property(fusb->usb_psy,
	...
	case FUSB251_STATE_DP_CONNECTED:
		dev_info(dev, "Display Port Connected\n");
		fusb->cc_mos_detected = false;
		fusb->sbu_mos_detected = false;
		break;
...
#ifdef CONFIG_TCPC_MT6360
	fusb->tcpc = tcpc_dev_get_by_name("type_c_port0");
	if (fusb->tcpc == NULL) {
		dev_err(dev, "%s: failed to get tcpc device\n", __func__);
		ret = -EPROBE_DEFER;
		goto free_dev;
	}
#else
...
#ifdef CONFIG_TCPC_MT6360
	ret = register_tcp_dev_notifier(fusb->tcpc, &fusb->psy_nb,
				TCP_NOTIFY_TYPE_USB);
#else
...
#ifdef CONFIG_LGE_DUAL_SCREEN
	if (check_ds_connect_state() >= DS_STATE_ACC_ID_CONNECTED
			&& fusb->state != FUSB251_STATE_DS_CONNECTED)
