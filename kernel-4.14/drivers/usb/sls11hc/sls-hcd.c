...
static unsigned int max_speed_hz = 1000000;
...
#ifdef VERBOSE_DEBUG
	print_hex_dump(KERN_INFO, "SPIWR", DUMP_PREFIX_OFFSET, 16, 1,
...
#ifdef VERBOSE_DEBUG
	print_hex_dump(KERN_INFO, "SPIRD", DUMP_PREFIX_OFFSET, 16, 1,
...
static int sls_check_unlink(struct usb_hcd *hcd)
{
	...
	unsigned long flags;
	int ret=0;//TD#691_(Using uninitialized value ret)
...
static void sls_handle_error(struct usb_hcd *hcd, u32 dw0)
{
	...
	dev_info(sls_hcd->dev, "%s: %s%s%s%s\n", __func__,
...
static void sls_host_transfer_done(struct usb_hcd *hcd)
{
	...
	if (!urb)//TD#914_(Dereference before null check)
		return;
...
static void sls_detect_conn(struct usb_hcd *hcd)
{
	...
	if (intr & SLS_PORT_REMOTE_WAKEUP)
		dev_info(sls_hcd->dev, "%s: REMOTE_WAKEUP\n", __func__);
	...
	if (!(sts & SLS_PORT_FULL_SPEED)) {
		dev_info(sls_hcd->dev, "%s: LOWSPEED\n", __func__);
		sls_hcd->port_status |=  USB_PORT_STAT_LOW_SPEED;
	} else {
		dev_info(sls_hcd->dev, "%s: FULLSPEED\n", __func__);
		sls_hcd->port_status &= ~USB_PORT_STAT_LOW_SPEED;
	}
