...
static int sw42902_power(struct device *dev, int ctrl)
{
	...
	case POWER_ON:
		if (atomic_read(&ts->state.core) == CORE_PROBE && ts->pmu == NULL) {

			ts->pmu = regulator_get(dev, "vtp");

			if (IS_ERR(ts->pmu)) {
				TOUCH_I("%s: Can't get vtp ldo regulator\n", __func__);
				ts->pmu = NULL;
			} else {
				TOUCH_I("%s: Found vtp ldo regulator.\n",__func__ );

				ret = regulator_enable(ts->pmu);
				if (ret) {
					TOUCH_I("%s: Failed to enable vtp ldo, ret=%d\n", __func__, ret);
				}
			}
		}
		if (param->touch_power_control_en == FUNC_ON) {
...
static int sw42902_lpwg_mode(struct device *dev)
{
	...
	if (ts->lpwg.screen) {
		...
		if (ts->lpwg.qcover == HALL_NEAR) {
			TOUCH_I("%s %d line - resume and hall\n", __func__, __LINE__);
		} else {
...
static void sw42902_connect(struct device *dev)
{
	...
	/* wire */
	if (charger_state == WIRELESS_CHARGER) //mtk wireless chargeer, charger_type.h
		d->charger = CONNECT_WIRELESS;
	else if (charger_state)
	...
	if (d->lcd_mode != LCD_MODE_U3) {
		if (d->lcd_mode == LCD_MODE_U2) {
			...
			case LCD_MODE_U0:
				...
				break;
			default:
				TOUCH_I("%s - LCD_MODE_U2_UNBLANK Mode change\n", __func__);
				break;
			}
...
static int sw42902_stop_pen_sensing(struct device *dev, bool enable)
{
	...
	u32 set_driving = 0;
	u32 addr = TC_CMD + tc_driving_ctl;
	u32 pen_stop_u3_driving = 0x8303;

	sw42902_reg_read(dev, addr, (u8 *)&curr_driving, sizeof(u32));

	if (enable) {
		prev_driving = curr_driving;
		set_driving = pen_stop_u3_driving;
	} else {
		set_driving = prev_driving;
	}

	if (curr_driving == set_driving) {
		TOUCH_I("%s : Do not need tc_driving\n", __func__);
		goto EXIT;
	}
...
EXIT:
	TOUCH_I("%s[%s] : curr_driving : %X, set_driving : %X\n", __func__,
		enable ? "T":"F", curr_driving, set_driving);
...
static int __init touch_device_init(void)
{
	...
	if (!is_lcm_name("BOE-SW43103")){
		TOUCH_I("%s, ddic BOE-SW43103 not found.\n", __func__);
		return 0;
	}
