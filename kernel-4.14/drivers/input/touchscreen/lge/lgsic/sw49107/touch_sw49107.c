...
#endif
#if defined(CONFIG_LGE_TOUCH_CORE_MTK)
//extern void mtkfb_esd_recovery(void); //bring up
#endif
...
int sw49107_display_power_ctrl(struct device *dev, int ctrl)
{
	...
	//primary_display_set_power_mode(ctrl); //bring up
#endif
...
static int sw49107_check_mode(struct device *dev)
{
	struct sw49107_data *d = to_sw49107_data(dev);
	int ret = 0;
...
//extern bool panel_type_touch(char *panel_name); //bring up
...
static int __init touch_device_init(void)
{
	...
	if (!is_lcm_name("LGD-SW49107")){
		TOUCH_I("%s, ddic LGD-SW49107 not found.\n", __func__);
		return 0;
	}
