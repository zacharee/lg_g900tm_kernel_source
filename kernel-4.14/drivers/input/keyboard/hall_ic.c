...
static int create_hallic_class(void)
{
	if (!hallic_class) {
		...
		if (IS_ERR(hallic_class)) {
			pr_err("[HallIC] class_create failed\n");
...
{
	int ret;
	pr_info("%s : hall_ic register + \n", __func__);
	if (!hdev) {
		pr_err("%s : hallic register failed\n", __func__);
		return -EINVAL;
	}

	hdev->dev = device_create(hallic_class, NULL, MKDEV(0, 1), NULL, hdev->name);
	if (IS_ERR(hdev->dev)) {
		pr_err("[HallIC] device_create failed\n");
		return PTR_ERR(hdev->dev);
	}

	ret = device_create_file(hdev->dev, &dev_attr_state);
	if (ret < 0) {
		pr_err("[HallIC] create dev_attr_state failed\n");
		goto err1;
	}
