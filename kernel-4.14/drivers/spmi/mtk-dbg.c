...
//static int is_drv_attr; //error: unused variable 'is_drv_attr' [-Werror,-Wunused-variable]
static char *wp;

#if 0 //error: unused function 'mtk_spmi_readl_d' [-Werror,-Wunused-function]
/*
...
{
	struct pmif *arb = spmi_controller_get_drvdata(dbg_ctrl);
	//error: unused variable 'log_size' [-Werror,-Wunused-variable]
	//unsigned int offset, tmp_dat, log_size = 0;
	unsigned int offset, tmp_dat = 0;
	unsigned int start, end;
	//int i = 0; //error: unused variable 'i' [-Werror,-Wunused-variable]
...
void spmi_dump_pmif_record_reg(void)
{
	...
	for (i = 0; i < 32; i++) {
		...
		tmp_dat = readl(arb->base + offset);
		//error: signed shift result (0x7C0000000) requires 36 bits to represent, but 'int' only has 32 bits [-Werror,-Wshift-overflow]
		chan = (tmp_dat & ((unsigned int)0xf8 << 27)) >> 27;
