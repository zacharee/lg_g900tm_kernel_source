...
#define PART_PATH "/dev/block/platform/bootdevice/by-name/efuse"
...
int get_partition_path(char *partition_name, char *path_name, int path_size)
{
	...
	snprintf(path_name, sizeof(path_name), "/dev/block/sdc%d", part->partno);
	put_part(part);

	return RET_OK;
}

static struct file *efuse_file_open(const char *path, int flags, int rights)
{
...
static int efuse_file_read(struct file *file,
			unsigned int offset,
			unsigned char *data,
			unsigned int size)
{
...
static int efuse_file_seek(struct file *file, unsigned int offset)
{
	int ret = 0;
	loff_t pos;

	pos = offset;
	ret = vfs_setpos(file, file->f_pos + pos, EFUSE_PARTITION_USED_SIZE );
...
int get_efuse_partition_data(unsigned char *result)
{
	...
	filp = efuse_file_open(PART_PATH, O_RDONLY, 0440);
	if (filp == NULL) {
		pr_err("Open EFUSE partition fail! errno=%d\n", ret);
		return ret;
	}

	ret = efuse_file_seek(filp, 0x0);
	if (ret != 0x0) {
	...
	ret = efuse_file_read(filp, 0x0, result, EFUSE_PARTITION_USED_SIZE);
