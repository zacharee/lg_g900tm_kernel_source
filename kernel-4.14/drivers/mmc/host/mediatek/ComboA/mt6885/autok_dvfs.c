...
int sd_runtime_autok_merge(struct msdc_host *host)
{
	...
	for (merge_count = 1; merge_count < AUTOK_VCORE_NUM; merge_count++) {
		if (host->autok_res[merge_count][0] == 0) {//error: comparison between pointer and integer ('u8' (aka 'unsigned char') and 'void *') [-Werror,-Wpointer-integer-compare]
	...
	if (merge_result == 0) {
		...
		host->autok_res[merge_count][0] = 0;//error: incompatible pointer to integer conversion assigning to 'u8' (aka 'unsigned char') from 'void *' [-Werror,-Wint-conversion]
		autok_tuning_parameter_init(host,
