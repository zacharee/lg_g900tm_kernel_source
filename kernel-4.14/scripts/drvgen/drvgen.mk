...
LGE_DT_NAMES := $(subst $\",,$(CONFIG_BUILD_ARM64_DTB_OVERLAY_IMAGE_NAMES))
LGE_DTS_DIR := $(dir $(LGE_DT_NAMES))
LGE_DTS_FILES := $(notdir $(basename $(wildcard $(srctree)/arch/$(SRCARCH)/boot/dts/$(LGE_DT_NAMES)*.dts)))
...
echo $(1) >>$(2);\
echo " id=$(my_dtbo_id)" >>$(2);\
lge_rev=`grep -m 1 "lge,pcb_revision.*;" $(addsuffix .dts,$(addprefix $(srctree)/arch/$(SRCARCH)/boot/dts/$(LGE_DTS_DIR),$(notdir $(basename $(1))))) | sed 's/\t//g' | sed 's/ //g' | sed 's/lge,pcb_revision=<//g' | sed 's/>;.*$$//g'`;\
echo " rev=$$lge_rev" >>$(2);\
$(eval my_dtbo_id:=$(shell echo $$(($(my_dtbo_id)+1))))
endef
endif
