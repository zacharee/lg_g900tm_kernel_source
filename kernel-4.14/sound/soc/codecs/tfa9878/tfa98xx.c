...
#define TOP_SPK_ADDR 0x34
#define BOTTOM_SPK_ADDR 0x35
...
#if defined(TFA_READ_BATTERY_TEMP) && defined(CONFIG_DEBUG_FS)
...
static int tfa98xx_info_spk_calib_status(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_info *uinfo)
{
	uinfo->type = SNDRV_CTL_ELEM_TYPE_INTEGER;
	mutex_lock(&tfa98xx_mutex);
...
static int tfa98xx_get_spk_calib_status(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct tfa98xx *tfa98xx;
	int mohm = 0;
	...
	list_for_each_entry(tfa98xx, &tfa98xx_device_list, list) {
		mutex_lock(&tfa98xx->dsp_lock);
		if (tfa98xx->i2c->addr == TOP_SPK_ADDR) {
			mohm = tfa_dev_mtp_get(tfa98xx->tfa, TFA_MTP_RE25);
			ucontrol->value.integer.value[0] = TFA_GET_BF(tfa98xx->tfa, MTPEX);
		} else if (tfa98xx->i2c->addr == BOTTOM_SPK_ADDR) {
			mohm = tfa_dev_mtp_get(tfa98xx->tfa, TFA_MTP_RE25);
			ucontrol->value.integer.value[1] = TFA_GET_BF(tfa98xx->tfa, MTPEX);
		}
		mutex_unlock(&tfa98xx->dsp_lock);
		pr_info("%s() : 0x%x, %d mohm, %ld\n", __func__, tfa98xx->i2c->addr, mohm, ucontrol->value.integer.value[tfa98xx->tfa->dev_idx]);
	}
	mutex_unlock(&tfa98xx_mutex);
...
static int tfa98xx_create_controls(struct tfa98xx *tfa98xx)
{
	...
	scnprintf(name, MAX_CONTROL_NAME, "SPK_CALIB_STATUS", tfa98xx->fw.name);
	tfa98xx_controls[mix_index].name = name;
	tfa98xx_controls[mix_index].iface = SNDRV_CTL_ELEM_IFACE_MIXER;
	tfa98xx_controls[mix_index].info = tfa98xx_info_spk_calib_status;
	tfa98xx_controls[mix_index].get = tfa98xx_get_spk_calib_status;
	tfa98xx_controls[mix_index].put = NULL;
	mix_index++;
...
static void tfa98xx_dsp_init(struct tfa98xx *tfa98xx)
{
	...
	if (failed) {
		...
		list_for_each_entry(ntfa98xx, &tfa98xx_device_list, list) {
#if defined(TFA_BYPASS_AT_START_FAILURE)
			if (ntfa98xx->tfa->spkr_damaged) {
				// dev_info(&ntfa98xx->i2c->dev,
				// 	"mute damaged device!\n");
				// tfa_dev_set_state(ntfa98xx->tfa, TFA_STATE_MUTE, 0);
				dev_info(&ntfa98xx->i2c->dev,
					"stop damaged device!\n");
				_tfa98xx_stop(ntfa98xx);
			} else {
#if defined(TFA_REDUCE_BYPASS_GAIN)
				int cur_tdmdspkg
					= TFA7x_GET_BF(ntfa98xx->tfa, TDMSPKG);
				...
				if (cur_tdmdspkg < TFA_REDUCED_TDMSPKG_IN_BYPASS) {
					...
					TFA7x_SET_BF(ntfa98xx->tfa,
						TDMSPKG, TFA_REDUCED_TDMSPKG_IN_BYPASS);

					/* reload configuration from scratch next time */
					ntfa98xx->tfa->first_after_boot = 1;
				}
#endif /* TFA_REDUCE_BYPASS_GAIN */
				mutex_lock(&ntfa98xx->dsp_lock);
				ntfa98xx->dsp_init = TFA98XX_DSP_INIT_FAIL;
				tfa98xx_set_dsp_configured(ntfa98xx);
				mutex_unlock(&ntfa98xx->dsp_lock);
			}
			...
			/* cancel other pending init works */
			cancel_delayed_work(&ntfa98xx->init_work);
			ntfa98xx->init_count = 0;
		}
