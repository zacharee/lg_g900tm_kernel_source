...
enum tfa98xx_error tfa_wait_cal(struct tfa_device *tfa)
{
	...
	for (i = 0; i < MAX_HANDLES; i++) {
		...
		if (cal_err != TFA98XX_ERROR_OK || calibration_done == 0) {
			...
			ntfa->is_bypass = 1; /* reset beforehand */
			err = tfa_cont_write_profile(ntfa,
	...
	*calibrate_done = 0;
	...
	if (!tfa->is_probus_device) {
		...
		while ((*calibrate_done != 1)
			...
			tfa98xx_convert_bytes2data((1 + 2) * 3 /* 1 + 2 data */,
				buffer, fw_status);
			...
			if ((fw_status[1] & 0x6) != 0) { /* damage event */
				for (i = 0; i < MAX_HANDLES; i++) {
					struct tfa_device *ntfa
						= tfa98xx_get_tfa_device_from_index(i);
					int damage_event = 0;
					...
					channel = tfa98xx_get_profile_tdmspks(ntfa);
					if (channel >= 2) /* up to stereo */
						continue;

					damage_event = (TFA_GET_BIT_VALUE(fw_status[1],
						channel + 1)) ? 1 : 0;
					if (damage_event) {
	...
	pr_info("%s: speaker damage flag 0x%06x:0x%06x\n",
		__func__, fw_status[1], fw_status[2]);

	if ((fw_status[2] & 0x6) == 0)
		return err;
	...
	for (i = 0; i < MAX_HANDLES; i++) {
		...
		ntfa->spkr_damaged
			= (TFA_GET_BIT_VALUE(fw_status[2], channel + 1))
			? 1 : 0;
...
enum tfa98xx_error tfa7x_status(struct tfa_device *tfa)
{
	...
	if ((tfa->daimap & TFA98XX_DAI_TDM)
		...
		if (TFA7x_GET_BF(tfa, TDMERR)
			|| TFA7x_GET_BF(tfa, TDMLUTER)) {
			pr_err("TDM related errors: STATUS_FLAG0 = 0x%x\n",
				TFA7x_READ_REG(tfa, TDMERR));
			pr_err("TDM related errors: STATUS_FLAG1 = 0x%x\n",
				TFA7x_READ_REG(tfa, TDMLUTER));
		}
	}
